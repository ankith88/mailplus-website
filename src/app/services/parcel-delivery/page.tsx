...
export const metadata: Metadata = {
  title: 'Parcel Delivery Sydney | Next-Day Metro | MailPlus',
  description: 'Fast, tracked parcel delivery from Sydney to all Australian capitals. Next-day metro, 2–5 day interstate. E-commerce integrations. From $8.50.',
  alternates: { canonical: 'https://mailplus.com.au/services/parcel-delivery' },
}

export default function ParcelDeliveryPage() {
  const service = getServiceBySlug('parcel-delivery')
  ...
  const breadcrumbSchema = buildBreadcrumbSchema([
    ...
    { name: 'Parcel Delivery', url: 'https://mailplus.com.au/services/parcel-delivery' },
  ])
  ...
  return (
    <>
      ...
      <ParcelDeliveryContent service={service} />
    </>
  )
}

function ParcelDeliveryContent({ service }: { service: ReturnType<typeof getServiceBySlug> }) {
    ...
    <>
      ...
      <div className="pt-32 pb-20 px-6 bg-earth-950 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            {service.name}
            <span className="italic text-earth-400"> from Sydney</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-4">
            ...
            >
              Send a Parcel — From ${service.startingPrice}
            </Link>
            <a
      ...
      <section className="py-20 px-6 bg-earth-900" aria-labelledby="features-heading">
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <span className="gradient-text italic">Every Parcel?</span>
          </h2>
      ...
      {/* Delivery Zones */}
      <section className="py-16 px-6 bg-earth-950" aria-labelledby="zones-heading">
        <div className="max-w-7xl mx-auto">
          <h2
            id="zones-heading"
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { destination: 'Sydney Metro', time: 'Next Business Day', highlight: true },
              { destination: 'Melbourne / Canberra', time: '1–2 Business Days', highlight: false },
              { destination: 'Brisbane / Gold Coast', time: '1–3 Business Days', highlight: false },
              { destination: 'Adelaide', time: '2–4 Business Days', highlight: false },
              { destination: 'Perth', time: '3–5 Business Days', highlight: false },
              { destination: 'Darwin / NT', time: '4–6 Business Days', highlight: false },
              { destination: 'Hobart', time: '2–4 Business Days', highlight: false },
              { destination: 'Regional Australia', time: 'Add 1–2 Days', highlight: false },
            ].map((zone) => (
              <div
                key={zone.destination}
                className={`rounded-2xl p-5 border ${zone.highlight ? 'border-amber-500/40 glass bg-amber-500/5' : 'glass border-earth-700'}`}
              >
                <p className="text-earth-300 font-medium text-sm mb-1">{zone.destination}</p>
                <p className={`text-base font-semibold ${zone.highlight ? 'text-amber-400' : 'text-earth-200'}`}>
                  {zone.time}
                </p>
              </div>
            ))}
          ...
          <p className="text-earth-500 text-xs mt-4">
            Transit times are estimates from date of lodgement. Remote area surcharges may apply.
          </p>
        </div>
      </section>
      ...
      <section className="py-16 px-6 bg-earth-950">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-earth-400 mb-8">
            Drop off at Waterloo or Alexandria depots. Free pickup for account holders. Instant label printing.
          </p>
          <Link
            href="/contact"
